...
public:
  ...
  int GetPingPong_CNT (void); //kihoon
  // int GetPingPong_CNT (void);
  int GetRSRPIndicator(void);
  double GetAverageSinr (void);

  int GetNumOfHO (void);
  std::map<uint16_t,int> GetRlfCounter (void) ;
  std::map<uint16_t,int> GetPreRlfCounter(void); //new
  void ClearRlfCounter (void) ;
  void ClearPreRlfCounter (void) ;
  void ClearTooLateHO_CNT (void);
  void ClearTooEarlyHO_CNT (void);
  void ClearWrongCellHO_CNT (void);
  void ClearNumOfHo (void);
  void ClearPingPong_CNT(void); //kihoon
  

  // void ClearPingPong_CNT(void);
  ...
  float previousrsrp =0;

  double Threshold_1 = -12; //To be changed //-7 
  double Threshold_2 = -14; // To be changed //-10 
  std::map<uint16_t, int> RlfCounter;
  std::map<uint16_t, int> PreRlfCounter;//0628
  int RSRPIndicator; //0528
  int MRO_TooLateHO_CNT = 0;
  int MRO_TooEarlyHO_CNT = 0;
  int MRO_WrongCellHO_CNT = 0;
  bool lateIndex = false;
  bool earlyIndex = false;
  bool wrongIndex = false;
  bool pingpongIndex = false;
  uint64_t hoIndex = 0;
  uint64_t prev_hoIndex = 0;

  // kihoon
  bool HandoverOccured = false;
  bool Ping_HandoverOccured;
  int handoverterm =0;
  bool canpingindex = false; 
  bool handovertimer = false;
  bool handovertermok =false; 
  bool handovertermgo =true; 
  uint16_t pre_PingpongCell = 0; //전전 셀
  

  // MRO
  int NumOfHo = 0;
  int PingpongCnt = 0;
  uint16_t PingpongCell = 0; // 이전 셀
 

  // For MRO state
  uint16_t best_rsrp = 0;
  uint16_t current_rsrp = 0;
  double GetCurrentRsrp (void);
  ...
  bool Insync = true;
  int RlfTimer = 0;
  bool ReEstablishment = false;
  int ReEstablsihmentTimer = 0;
  int RrcRejectNumber = 0;
  int RrcReestablishmentNumber = 0;
